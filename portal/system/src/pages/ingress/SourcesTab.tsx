  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto font-mono text-[13px]">
        ...
        {sources.map(s => (
          <div key={s.id} className="grid gap-4 px-5 border-b border-border hover:bg-white/[0.02] items-center transition-colors min-h-[52px] py-2" style={{ gridTemplateColumns: cols }}>
            <div className="font-sans font-medium truncate" title={s.name}>{s.name}</div>
            <div className="text-[11px] text-accent truncate" title={s.stream}>{s.stream}</div>
            <div>
              <button
