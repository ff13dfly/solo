...
interface Sentinel {
  ...
  createdAt: number;
  online: boolean;
  // §1.2 — which permit the sentinel's fetchers/autorun run under: the shared
  // system.nexus relay, or its own system.* bot (with token provisioning state).
  identity?: { mode: 'bot' | 'shared'; uid?: string; hasToken?: boolean | null; expired?: boolean; expiresAt?: number | null };
  // Activity ledger written by the stream consumer — fired (guard passed, delivered),
  // skipped (guard filtered), failed (delivery aborted/retried).
  activity?: { fired: number; skipped: number; failed: number; lastFiredAt: number | null; lastFailedAt: number | null } | null;
}

const TRACK_OPTIONS = ['internal', 'external'] as const;
const REACHABILITY_OPTIONS = ['', 'built-in', 'polling', 'sse', 'webhook'] as const;
const ON_ERROR_OPTIONS = ['abort', 'skip', 'fallback'] as const;
// Mirrors api/core/nexus/logic/context.js READ_SUFFIXES — gives instant feedback
// before the backend's authoritative read-only gate rejects a write method.
const READ_SUFFIXES = ['get', 'list', 'query', 'search', 'count', 'resolve', 'info'];

// One row of the data_fetchers editor — all fields are raw strings, parsed on submit.
interface FetcherRow {
  ...
  on_error: typeof ON_ERROR_OPTIONS[number];
  fallback: string;     // JSON (only when on_error === 'fallback')
  guard: string;        // JSON (optional per-fetcher guard)
}

const emptyFetcher = (): FetcherRow => ({
  key: '', method: '', params: '', result_path: '', depends_on: '', on_error: 'abort', fallback: '', guard: '',
});

// Reverse of buildContext()'s fetcher serialization — turns a stored data_fetcher
// back into an editor row so a Sentinel can be edited.
const fetcherToRow = (f: any): FetcherRow => ({
  key: f.key || '',
  method: f.method || '',
...
// ─── Sentinel Permit Modal ─────────────────────────────────────────────────────
// Read-only view of the permissions BEHIND a sentinel: which identity it runs as,
// what its context DECLARES it needs (fetcher methods + agent.decide when autorun),
// and whether the bot's permit actually grants each — the pre-audit verdict
// surfaced in UI, before a runtime abort/DLQ reveals the gap. Editing stays in
// BOT ACCOUNTS (single source of truth: the user service's bot permit).

function declaredNeeds(s: Sentinel): string[] {
  const needs: string[] = [];
...
function SentinelPermitModal({ sentinel, onClose }: { sentinel: Sentinel; onClose: () => void }) {
  const { t } = useLang();
...
// ─── Sentinel Deliveries Modal ────────────────────────────────────────────────
// The minimal "inbox view" slice: every guarded delivery lands in the notification
// inbox (audit trail), including the autorun verdict — decision / confidence /
// escalate / autorun_error. escalate=true is the AI→human hand-off; before this
// modal it sat invisible in redis (the v2-deferred "agent 收件箱视图" pain point).
...
function SentinelDeliveriesModal({ sentinel, onClose }: { sentinel: Sentinel; onClose: () => void }) {
  const { t } = useLang();
