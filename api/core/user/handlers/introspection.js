...
module.exports = [
    ...
    // salt/hash were CONSUMED but never DECLARED: omitting them made the service mint a random
    // credential nobody holds, and since there is no password-reset method, that account could
    // never log in — while register still answered { success: true, uid }. Anyone coding from the
    // machine-readable declaration (an AI agent especially) hit it; only the prose GUIDE §2b warned.
    // Now declared AND required, so the perimeter rejects it instead of minting garbage.
    // docs/feedback/done/operator-onboarding-three-silent-traps.md §一
    { name: 'user.register', params: [{ name: 'name', type: 'string', required: true, maxLength: 128 }, { name: 'salt', type: 'string', required: true, maxLength: 128, description: 'Client-generated random salt (hex). Kept so the client can recompute hash at login.' }, { name: 'hash', type: 'string', required: true, maxLength: 256, description: 'Client-computed password hash bound to salt — the server never sees the password and CANNOT recover this. See router GUIDE §2b for the exact derivation.' }, { name: 'email', type: 'string', maxLength: 254, pattern: 'email' }, { name: 'phone', type: 'string', maxLength: 32, pattern: 'phone' }], returns: ['success', 'uid'], returns_schema: [{ name: 'success', type: 'boolean', required: true }, { name: 'uid', type: 'string', required: true }], description: 'Register new user. salt + hash are mandatory and client-computed (see router GUIDE §2b) — there is no password-reset path, so an account registered without them can never log in.', ai: true, public: true },
    ...
    // authoritative table); page/pageSize is the legacy dialect portal/system still sends, and
    // logic/user.js folds both through library/pagination.js. limit / keyword were accepted by
    // the handler all along but never declared — undeclared params are invisible to the Router's
    // capability catalog, so an AI agent reading `methods` could neither page nor search.
    // The RETURN keys stay page/pageSize (portal reads result.pageSize) — that's a separate change.
    { name: 'user.account.list', params: [{ name: 'includeDeleted', type: 'boolean' }, { name: 'keyword', type: 'string', maxLength: 256 }, { name: 'limit', type: 'number' }, { name: 'offset', type: 'number' }, { name: 'page', type: 'number' }, { name: 'pageSize', type: 'number' }], returns: ['users', 'total', 'page', 'pageSize'], returns_schema: [{ name: 'users', type: 'array', required: true }, { name: 'total', type: 'number', required: true }, { name: 'page', type: 'number', required: true }, { name: 'pageSize', type: 'number', required: true }], description: 'List users, paginated (prefer limit/offset) + optional keyword search (Admin only)', ai: true },
