    ...
    description: {
        en: {
            ...
            methods: {
                ...
                'fulfillment.profile.submit':      ['Submit a NEW profile into the review lane (CREATES it; lint-gated → PENDING_REVIEW; not usable until approved). With { id, enroll: true } (admin): move an existing trusted profile into review instead'],
        ...
        zh: {
            ...
            methods: {
                ...
                'fulfillment.profile.submit':      ['投稿一个新配置模板进入审核通道（是创建，不是提交已有模板；lint 把关 → PENDING_REVIEW）；审批前不可用。带 { id, enroll: true }（管理员）则把既有可信模板转入审核'],
