...
- **fleet-standard 系统方法共 5 个**：`ping`/`methods`/`entities`/`events`/`guide`（2026-07-23）。`guide` 返回服务目录 `GUIDE.md`（任务配方，与代码同 commit 更新；`api/library/guide.js`），**只注册不进 introspection 声明**；外部 AI 代理第一跳 = 匿名 `system.guide`（router 下发认证流程 + 信封 + 错误码），`system.guide {service}` 生产需认证。背景见 `docs/feedback/done/ai-agent-self-describing-api.md`。
