...
> **本文写于规划期**（校对基准 2026-06-03，基于当时代码的精确核查含 file:line），**当时尚未实现；系统此后已完整落地**——
> 现有 **66 套** suites（编号 `00-login` → `113-passport-identity-line`），外加 `e2e/ui/` 的 Playwright UI 层，CI 每次推送都跑。
> 因此下文 §9「分阶段实施」、§13「实现前定下的决策」、§14 记录的是**当时的规划与首次落地状态**，作为背景保留、不再更新；
> **当前真实覆盖以 `e2e/suites/` 目录和 CI 的 e2e job 为准**（`.github/workflows/ci.yml`）。
...
> 📌 **编号只表达「大致概念时序」,不保证唯一** —— 目前 `54 / 66 / 67 / 69 / 70` 各有两套同号文件。
> Jest 按 glob `suites/**/*.e2e.test.js` 收集,**重号不影响执行**;既然每个文件自洽、文件间不共享状态,
> 重排文件名的收益只有阅读顺序,却要打断 git 的文件历史追踪,故**刻意不改**。定位用文件名后半段(主题词),别靠序号。
...
## 14. 首次落地记录（2026-06-03 快照 — 不是当前状态）

> ⚠️ **本节数字是首次跑通时的快照，早已不是现状**（当时 full 档 17 套，现在 `e2e/suites/` 有 66 套）。
> 保留它是为了记录「第一次全绿时系统长什么样」；**要看当前覆盖与结果，数 `e2e/suites/` 的文件、看 CI 的 e2e job**，
> 别引用下面的数字。

**（当时）已实现并跑通(lite + full 两档全绿)。** `e2e/` 是独立 Jest 项目(`cd e2e && npm install`).
