...
- **现状（校准到 2026-09-05）**：Router 写入端**已修剪**——`MAXLEN ~`，默认约 1 万条，
  `EVENT_MAXLEN` 可调；单条流另用 `EVENT_MAXLEN_OVERRIDES='EVENT:MARKET:CANDLE_1M=100000,…'`
  覆盖（流名原样写）。`MAXLEN ~` 是**近似**修剪：Redis 按基数树节点边界裁，实际保留条数
  **不少于**阈值——精确修剪每写一次都是 O(n)，不值得。
  设 `EVENT_MAXLEN=0` 可显式关掉修剪（回到 2026-09-05 之前的无界行为）。
  ⚠️ **升级注意**：此前无界长起来的存量流，升级后**首次写入即被裁到阈值**。真把流当账本用过
  的，升级前先把历史导出去。量级参考：仅 2 个 symbol 的 1m+5m K 线事件 ≈ 0.86 MB/天。
  `NEXUS:DLQ`（投递失败停车场）同样已封顶（`NEXUS_DLQ_MAXLEN`，默认 1000）。
- **对账写法**：拿 `XLEN` 绝对总数与业务账本对账的用法，**修剪上线后已经会静默失真**——
