...
下面目录树里标 `[Solo]` 的东西升级时整体覆盖——这常被读成"改了会丢，所以小心点"。
**它真正的作用是「标准不由使用方调整」。**

一般框架的"最佳实践"只是文档里的一句话，改了不会有任何后果；**Solo 的只读区有后果**，
所以它是真的标准，不是建议。这正是集装箱能成立的地方：ISO 箱规六十年不变，
价值全部来自**它不变**。

在「多人 + 每人各自的 AI」这个用法下，这是唯一能保证**每个人的 AI 不会各自发明一套约定**
的东西——而那恰恰是 AI 辅助开发规模化时的头号风险。配套的执行细则是：
**框架级问题回写 solo 的 `docs/feedback/`，而不是在本地打补丁**（这条路是通的，
多批下游反馈已被上收成框架能力）。
