...
| **DOM 提取链路遗留（2026-08-22 triage）** | steward 反馈落地后剩三条框架级（当日已修的不在此列：resolveParams 数组塌陷 ✅、instance.list sourceId 声明 ✅、runbook 重写 ✅）：① **`fulfillment.instance.create` 无 sourceId 幂等**——同 sourceId 重投建重复实例，at-least-once 投递方（extension-kit 队列、_tasks 重试）都受影响；现由客户端 `list(sourceId) 先探再建` 兜着，服务端加 `idempotent: true`（同 sourceId 返回既有实例）才是正解。② **agent 文本方法 4000 字符上限无长文本档**——DOM/长文提取只能靠端侧蒸馏；要么开大某个专用方法的上限、要么走 storage 引用（仿 gateway 附件方案）。③ **`instance.pending_callbacks` 死字段**——建了从未读写，要么接 workflow 回调、要么删。 | `docs/feedback/runbook-browser-extension-ai-extraction-not-runnable.md`（处理结论）、`docs/runbook/browser-extension-ai-extraction.md` |
