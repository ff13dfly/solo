...
| ~~2.3~~ | **✅ 外部投递闭合（webhook 已建 / sse fail-closed，2026-06-10）** | `gateway.webhook.send`（`gateway/logic/webhook.js`：POST JSON + HMAC-SHA256 `X-Solo-Signature` 签名 + SSRF 守卫 + 有界超时）已注册；sse 改为 `notification.config.set` / `sentinel.broadcast` 直接 fail-closed（"配上即死信"→"诚实拒绝"），broadcast 的 webhook 路径随之转真。剩：主动推送面已拉回 v1.1.x（2026-07-03 D 线），形态 2026-08-23 具象为 **WS 门铃**，见 §3「对外推送面」。 | `gateway/handlers/introspection.js:127` + `index.js:130`；hermetic `gateway/tests/webhook.test.js`；e2e `100-delivery` |
...
| **生产硬化** | 2026-07-30 复核：原清单大半已 stale——relay/category 超时 ✅（06-07）、permanent-error 分类 ✅（06-10）、storage 签名 URL ✅（driver 化）、**CORS ✅**（`library/cors.js` env 驱动全队接线，router 刻意除外）、**`/metrics` ✅**（`library/health.js` Prometheus + notification/nexus/orchestrator 业务 gauge——toFix 里"未做"是旧账）、**Redis 密码 ✅（2026-07-30：scaffold init.sh 生成 `REDIS_PASSWORD` + REDIS_URL 内嵌 + run.sh `--requirepass` + `REDISCLI_AUTH`，无密码 .env 保持旧行为）**。**剩一件真的**：Router token jti 级反重放——Ed25519 确定性签名 + 毫秒 iat → 无 jti 则"同毫秒合法并发"与"窗口内重放"不可区分，**必须 router `forward.js` 加一行随机 jti**（待授权；iat 新鲜度窗口已把重放压到秒级，风险有界）。storage 另有 wavely 反馈的 visibility 语义修复 ✅（docs/feedback/done/storage-visibility-semantics.md 处理结论）。 | `docs/planning/toFix.md §2`（注意 stale）+ 本行 |
...
| **gateway 出站缺口** | **两轮补齐（2026-07-30）：19 条中 16 条已修，只加不破**。首轮：aliyun V3 签名（原方案 `Authorization: AccessKeyId` 无效 + HTTP 200 业务码失败当成功）· Twilio variableOrder 位置变量 · api 通道诚实收敛为 Resend 适配器表 · 脚手架 `GATEWAY_SECRET_KEY` · 发前校验（模版必填 + 收件人格式 → `-32602` 直接 DLQ）· **投递台账 `gateway.delivery.*`** · **幂等键（notification worker 已接线）** · 投递事件（`_event` 夹带）· cc/bcc/replyTo · 模版 text · opt-in 严格变量 · clock 清零。第二轮（`system.gateway` relay bot 一份基建四收益）：**附件（storage 引用 + 总量 cap，拒裸 base64）** · **失败事件（relay event.emit）** · **回执回流 `delivery.update`（SENT→DELIVERED/BOUNCED/COMPLAINED）** · **通道探针 `channel.test`（四 provider 只读，不真发）**；router 默认事件注册表加 gateway 两行（2026-07-30 授权）。**剩 3 条**：G9 出站配额（a 档 v1.1.x）· G10 portal 页 · G11 多通道账号；另欠 aliyun/Resend **真机实测**（等凭证）。 | `docs/planning/gateway-gaps.md`（带 file:line + 逐条修法/验证/证据） |
| **DOM 提取链路遗留（2026-08-22 triage）** | steward 反馈落地后剩三条框架级（当日已修的不在此列：resolveParams 数组塌陷 ✅、instance.list sourceId 声明 ✅、runbook 重写 ✅）：① **`fulfillment.instance.create` 无 sourceId 幂等**——同 sourceId 重投建重复实例，at-least-once 投递方（extension-kit 队列、_tasks 重试）都受影响；现由客户端 `list(sourceId) 先探再建` 兜着，服务端加 `idempotent: true`（同 sourceId 返回既有实例）才是正解。② **agent 文本方法 4000 字符上限无长文本档**——DOM/长文提取只能靠端侧蒸馏；要么开大某个专用方法的上限、要么走 storage 引用（仿 gateway 附件方案）。③ **`instance.pending_callbacks` 死字段**——建了从未读写，要么接 workflow 回调、要么删。 | `docs/feedback/done/runbook-browser-extension-ai-extraction-not-runnable.md`（处理结论）、`docs/runbook/browser-extension-ai-extraction.md` |
| **对外推送面（WS 门铃，2026-08-23 triage）** | 事件总线消费端全在信任域内（matcher/nexus 两消费组），域外客户端（浏览器插件/portal）只能轮询（30s 地板、平均 15s 延迟）。**语义已拍**：notify-only 门铃（推「有新事件」+ stream 条目 id 做 cursor，数据仍走既有 RPC 拉取；at-most-once、断线不补投、可合并、降级=轮询）+ 可见性**默认全关、env 白名单按流开放**（沿 xadd triage 的 env 列表先例，如 `EVENT_EXTERNAL_STREAMS`，不动 registry 结构）。**形态倾向 Router 内建** `server.on('upgrade')`（`router/index.js:440` 返回值现未捕获，同端口零新增入口）但属 router 保护区——**待用户授权**，与 EVENT:* TRIM 接线（xadd 反馈建议 1/2）同批。它是 D 线「SSE 推送」排期项的具象化，形态改 WS（MV3 SW 保活需双向活动）。events.md 契约表述随实现落地，不提前写。 | `docs/feedback/event-bus-no-external-push-plane.md`（处理结论） |
| **bundle 运行时自报版本（2026-08-24 定）** | 🔴 **线上跑的是哪版 bundle，wire 上无法判定**——各 core 服务 `config.js` 读**自己那份** per-service `package.json`（全部 `0.1.0`、从不步进，esbuild 内联进 bundle），消费者栈里任何 core 服务 ping/`/health` 报的版本**永远是 0.1.0**；router 的 config 连 version 字段都没有（ping 返回 undefined）；`.solo-version` 只在 `run.sh:133-135` 选 bundle 文件名，不进运行时——文件名可撒谎且无检测（run.sh 只查了前端漂移，`run.sh:508`）。**三个消费者等着它**：① v2 bridge「握手带版本」（`VERSION.v2.md` §3.3，2026-08-23 已采纳）没有运行时数据源；② 主箱定期拉取的 L0「三处版本对齐」指标（`v2-bridge-interaction.md` §3）无法自动化；③ mso 型「线上切没切判断不了」事故在 solo 栈无解。**最小改法（只加不破，不碰 `api/router/`）**：`deploy/build.sh` esbuild 注入构建版本（`--define` 或 gen-entry 播 `global.__SOLO_BUNDLE_VERSION__`，`__SOLO_PORTS__` 同款先例），`api/library/health.js` 的 `mountHealth` 读全局往 `/health` 加 `bundle` 字段——所有服务（含 router，它也调 mountHealth）自动带上；from-source 启动无 bundle 则如实报 null。⚠️ 各服务 ping 里的 0.1.0 **保持不动**（那是「服务自己的版本」语义），新增 `bundle` 字段单独回答「底座是哪版」。时机：下一个 patch 顺手做；**bridge 统计航线（定期拉取）动工前必须落**。 | 本行 + `v2-bridge-interaction.md` §3（L0 指标）、`VERSION.v2.md` §3.3（握手带版本） |
| **bridge 消息档 v1：`gateway.webhook.send` solo 目标模式（2026-09-03 定）** | v2 A 线收敛后 frame 唯一要加的东西（`VERSION.v2.md` §3.0）。现状：gateway 出站发裸 JSON `{type,targetId,payload,sent_at}` + HMAC 头，ingress 只收 JSON-RPC `ingress.ingest {request_id,data}` + `Authorization: ApiKey` 头——**同一 frame 的出口与入口不说同一种话**，两个 SOLO 箱子今天不能直插（`webhook.js:45-54` vs `ingest.js:97-128`）。**改法（只加不破，v1.x）**：`gateway.webhook.send` 加 solo 目标模式——① 外壳改 JSON-RPC 信封、`request_id` 取 `idempotencyKey`（发方账本与收方去重一个键贯穿）② 鉴权改 `Authorization: ApiKey <secret>` 头 ③ 解析回执，`duplicate:true` / 422 held 如实透出（现在只看 2xx）④ **放行 loopback**（仅 solo 模式且路径 `/jsonrpc`——同机多箱是 A 线第一里程碑，`webhook.js:40-42` 的闸会拦死第一跳）⑤ 信封 `meta.hop`，ingress 透传进事件 payload，`hop ≥ 3` 拒发（环路刹车，§3.6 #5 唯一需要的机制；ingress README §15 D2 的 `meta` 首个字段）。⚠️ 顺带核实 Router 审计是否记 params——记则 `secret` 应改为引用（ingress 当初让 key 走头不走 params 正是为此）。**不做**：签名 / principal / 预检 / 独立服务（跨运营方档）、RPC 档（当对方 Router 普通客户端即可）。第一个消费者 steward → finance；同机形态 overview → runner。 | `docs/feedback/gateway-ingress-dialect-mismatch.md`、`VERSION.v2.md` §3.0、`v2-bridge-interaction.md` §2/§6 |
| **agent provider 能力面（2026-08-29 triage）** | awareness 反馈落地后剩三条（当日已修的不在此列：chat `messages` / text.parse `schema` 契约补全 ✅、限流 bot 语义入 description ✅）：① **`messages` 跨 provider 归一化**——现状 gemini 独有，qwen 认自己的未声明参数 `history`、openai 只认 `text`；归一后 system prompt 才真正 provider 无关（qwen/openai 侧把 messages 映射进各自请求形状，qwen 的 `history` 顺带收编或废弃）。② **ark（火山方舟）provider**——或实测 `OPENAI_BASE_URL`（config 注释 RESERVED、验证范围现为零）接 OpenAI 兼容端点后如实写明范围；等 awareness 真接时以实测回填。③ bot 身份限流单独计额（`by:'user'` 对 relay 调用 = 整服务共享 5/min）——涉 Router checkAccess 侧（红线），真实撞限再议。④ gemini.js 两个重名 `classifyImage`（前者死代码被后者遮蔽，2026-08-30 吞错 triage 时发现）待清理。⑤ 中央模型表里的 `gemini-2.0-flash-exp`（image.generate）与 `gemini-embedding-2` 未验证是否仍在线——1.5-flash 死透的同型风险，下次撞 404 先查这。 | `docs/feedback/done/agent-chat-contract-hides-provider-capability.md`、`done/agent-provider-swallows-errors-as-success.md`（处理结论） |
| **实体读写面：二级索引 + 同病残点（2026-08-30 triage）** | finance 反馈落地后剩三条（当日已修的不在此列：`listAll()` 一等语义含迁移自愈 ✅、`truncated`/`onTruncate` 截断响亮化 ✅、`batchSize` 语义入契约 ✅、软删标准补 `softDelete: false` 出口 ✅、`search.js` 示例不再教 `limit: 9999` ✅）：**写侧已于 v1.2.9 收口**（`createMany`/`deleteMany` 一个 chunk 一次 MULTI，20 万行重导一轮 7.46s 落进 Router 10s 预算；`destroy` 游标索引泄漏同批修掉）。剩：① **二级索引（v2）**——按声明字段维护 `SERVICE:ENTITY:BY:<field>:<value>` 集合之类，解「过滤即全扫」：现在无论 offset 还是 cursor 路径，filter 都在读之后跑，报表要一个月的数据也得翻完全部历史。账面上它是唯一让「只读一个期间」的成本与期间大小成正比的路，但成本最高、收益依赖量级 ⇒ 排最后。② 🔴 **`orchestrator/logic/workflow.js:777` 同款 magic number**——`build()`（给 agent 建 workflow 快照）用 `list({ limit: 1000 })`，超出静默丢失 = AI 能力面悄悄缺项；它的 `list` 是手写实现（`:360` 直接 `redis.json.get` + 自有索引）**不走 Entity Factory**，`listAll()` 够不着，需自己的修法。当前量远小于 1000，潜伏风险非现症。③ **churn 型实体的墓碑清理**——存量已软删数据（实测某实体 87% 键为 DELETED）改声明 `softDelete: false` 只影响新写入，旧墓碑要一次性清理脚本才走得掉。 | `docs/feedback/done/entity-list-fetch-all-semantics.md`（处理结论）、`api/library/entity.js` |
| 🔴 **Router 转发超时不取消下游 = 假失败（2026-08-30 发现）** | `api/router/handlers/forward.js:83` 的 `axios.post` 只设了 `timeout`（默认 10s，agent 90s / gateway 60s 例外），**没有 AbortController/signal**：超时只是 router 这端不等了，下游 Express handler 不会被取消、会把活干完。⇒ **调用方收到 `-32099` UPSTREAM_ERROR，操作其实成功了**——比静默失败少见但同样危险，且症状极具误导性（重试会二次写入）。finance 21000 行导入实测踩到，当时用「作业记录 + 状态查询」兜住（那是补丁不是解法）。v1.2.9 的 `createMany` 把撞墙的行数区间推远了一个数量级（重导 20 万行 7.46s），**但机制本身仍在**，任何足够慢的方法都会再撞上。**修法属 router 保护区、待授权**：给 forward 的 axios 调用挂 AbortController，超时时 abort 让下游连接断开（下游还需自己响应 `req.on('close')` 才真停）；或在契约上承认它——超时错误里带「操作可能已生效」的语义，让调用方走幂等键而不是盲目重试。后者不动 router 也能做一半。 | `docs/feedback/done/entity-factory-no-bulk-write.md`（处理结论 §核实结果）、`api/router/handlers/forward.js:11,83` |
| **WAL 落盘面残项（2026-08-30 二轮 triage）** | 批量归档 + 按天分片已于 v1.2.9 落地（丢审计修复、存储降 10 倍）。剩两条，均非阻塞：① **`logger.query(key)` 现在要扫索引文件**——旧布局是 md5 直接算路径 O(1)，新布局按 `key` 在每日 `.index` 里线性找，再按偏移 seek。单日 20 万行的索引约 20MB，审计查询能接受，但**没有按 key 的二级索引**；量再上一个台阶就得给索引建索引（与实体面的「二级索引」是同一类问题，可一并考虑）。~~② 按天日志无保留/轮转策略~~ **✅ 已解决（2026-08-30）**：`deploy/wal-rotate.sh`（压缩 + 可选冷存搬运 + 可选 Redis 快照，gzip 实测 13:1）+ archiver 磁盘水位告警（`archiver.disk.low/critical`，`WAL_DISK_*` 可调）+ `query()` 认得 `.gz`（轮转不致历史静默消失）。**剩的是接线**：给长期运行的栈挂上定时（Linux systemd timer；macOS 挂常驻栈生命周期，别用 cron/launchd）。 | `docs/feedback/done/entity-factory-no-bulk-write.md`（二轮处理结论）、`api/library/logger.js` |
| **依赖漏洞清零（2026-09-04 基线）** | CI 此前零漏洞扫描；当日 `npm audit --omit=dev` 扫 api/ 生产依赖：**6 high + 1 moderate + 1 low**。分两档：① **非 major、`npm audit fix` 即修**：axios（直接依赖，表单序列化 maxDepth 绕过 + 原型污染）、http-proxy-middleware（直接，CRLF 注入）、form-data（间接，CRLF）、urllib（间接，跨域重定向带凭证头）、qs（moderate，DoS）——修完必须过 CLAUDE.md §6.1 锁文件门禁（隔离目录 `npm ci`）+ 全量 jest；② **major、要各自验证**：nodemailer ≤9.0.0 → 9.1.1（gateway SMTP 路径，`core/gateway/tests/smtp-*.test.js` + 真发一封）、sharp <0.35 → 0.35.4（storage 图像路径，libvips 四个 CVE）。清零后把 ci.yml static job 里 high 那步的 `continue-on-error` 删掉、变阻塞。Dependabot 已开（`.github/dependabot.yml`，api/ 每周一批），之后新漏洞会自己来 PR。 | `.github/workflows/ci.yml`（static job 两步 audit）、`.github/dependabot.yml`、`.github/workflows/codeql.yml` |
| **orchestrator step `condition` 未纳入 fail-closed** | `library/jsonlogic.js` `evaluateCondition` 2026-09-04 起对 `<` `<=` `>` `>=` 的缺失字段 fail-closed（fulfillment 守卫 + nexus guard 受益）。但 `core/orchestrator/logic/runner.js:6` **直接 require json-logic-js** 做 step `condition`，同样的 `null >= null → true` 在那里还在：`{">=": [{"var":"step.load.result.count"}, 1]}` 遇上游没返回 count 时会**执行**该 step 而不是跳过。修法：runner 改用 `library/jsonlogic.evaluateCondition`（语义一致，且去掉一处直接依赖），加 `engine.test.js` 用例钉住；orchestrator 属 core 非 router 保护区，可直接做。另：`deploy/build.sh:95` 的 `npx esbuild` **未钉版本、每次 CI 从 registry 拉**——upgrade-path job 与 init.sh 都依赖它，esbuild 大版本变更会同时打红两处；应加进 api/ devDependencies 钉版（走 §6.1 锁文件门禁）。 | `core/orchestrator/logic/runner.js:6`、`api/library/jsonlogic.js`、`deploy/build.sh:95` |
...
> CI 白名单（`api/jest.ci.config.js`）**列表即计数**（`node -p "require('./jest.ci.config.js').testMatch.length"`；
> 2026-09-04 实跑 133 套 / 2175 测试，2170 passed + 5 skipped）。仓内 `*.test.js` 162 个，余 29 个被排除
> （18 个要外部 LLM、4 个要活网格、3 个 process.exit 脚本式约定、3 个实测红、1 个 fulfillment/generate 无记录原因）。
> 下面是把门禁做厚的子集，均"只加不破"。
...
| 5.7 | ✅ **三类盲区各补一道门**(2026-09-04) | 盘点 v1.2.8→v1.2.12 十一处修复：5 处来自下游生产报告、1 处 CI 抓到（红两天没人看）；类别 = 资源生命周期 ×4、契约 ×4、升级路径 ×2、并发 ×1——现有套件全按「单次调用对不对」断言，对**累计不守恒、升级覆盖、依赖漏洞**三类是盲区。补：**① `upgrade-path` job**（`deploy/check-upgrade-path.sh`，init → 三区哨兵 → 伪装旧版 → upgrade ×3 → 49 断言 + doctor；init.sh/upgrade.sh 此前零覆盖）**② e2e `105-wal-conservation`**（N 条写入后 SET/ZSET/热流/磁盘四处守恒，reclaim 过冲与丢行在此现形）**③ `autocheck/simulation` 七场景接进 `test` job**（此前只查文件存在、从未执行；修了 orchestrator 场景两处腐烂、删 authority 死场景）**④ 安全零成本档**（npm audit critical 阻塞 / high 报告、dependabot、CodeQL）。**未做**：gateway↔ingress 契约测试（等 §3 bridge solo 模式一起）、旧 bundle 对新 Router 的版本错开 e2e、fast-check 属性测试、Redis 中途重启的故障注入、CI 红了无人看的通知（GitHub 侧设置，仓库里做不了）。 | `deploy/check-upgrade-path.sh`、`e2e/suites/105-*`、`api/autocheck/simulation/run.js`、`.github/` |
