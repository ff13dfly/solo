...
- **异构成员资格 / edge 规范是否独立成文**（2026-08-29 提出）。**定性先立住**：§3.3 的四件套
  （mesh 签名认证、外网格 principal + 窄 permit、capability 握手带版本、诚实错误码）全部是
  **边界契约**，没有一条预设对端跑 SOLO bundle——「每个网格掌管自己的身份、对端自己决定信不信」
  本就是主权系统互认的形状，主权联邦天然容忍异构。任何非 SOLO 系统只要立一个实现这四件套的
  「使馆」endpoint 即可入联邦；v1 先例现成：ingress / gateway / MCP adapter 就是三个 adapter 级
  异构边界器官，bridge = 同一模式抬到 peer 级 RPC。**但整体强度要诚实分层**：同 frame 联邦 =
  互懂 + 共同演进 + 互操作（强整体）；异构联邦 = 协议整体（只剩互操作 + 归因 + 授权边界）——
  五要求里只有 R4 跨 frame 族存活，R1/R2/R3/R5 塌缩为各族内政，无法替非 SOLO 端保证其 AI 被
  治理、其分叉可见（论文 §2.3 的 mutual intelligibility 明确不跨族转移，划界已对，不必改）。
  **待拍板的就一件事**：把边界契约独立成一份「mesh edge 规范」（envelope、签名、capability 表
  shape、错误码、版本握手——任何系统照读可实现），还是让它作为 bridge 服务源码的隐含行为存在。
  前者决定异构方加入时是「读协议」而不是「逆向 SOLO」，也直接让论文 §7「第二个独立实现」那个
  测试便宜一个量级。**现实驱动已到场**：steward 面对两家公司 = 两个独立信任域的 SOLO，异构/
  跨运营方问题的第一个真实实例——版本必然不齐、无权替对方升级，§3.3「版本握手」的预判在此成立。
