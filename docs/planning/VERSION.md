...
>
> ⚠️ **v1.1 已封板；本文自此是历史切面，不再随后续版本更新。** v1.2.0（2026-08-20）在其之外
> 新增了 `client/extension-kit/`（浏览器插件的框架侧半边）——它是**新交付物**而非 v1.1 边界内
> 的待办，判定只加不破故未触发 v2。后续版本内容一律看
> [`CHANGELOG.md`](./CHANGELOG.md)，别在本文里找。
