...
- **单位**（2026-08-26 修订）：组织单位是**箱子**（标准化技术栈 + 人类归属），**不是「1人1AI」配对**——一个箱子可以 n 人 n AI（user 服务本就多账号，AI 入口有交互会话/定时采集/sentinel/经 system.guide 与 MCP 进来的外部 agent），一个人也可以拥有多个箱子（现状：多数箱子同一运营者）。模型钉死的是边界与归属，不是人头数。
